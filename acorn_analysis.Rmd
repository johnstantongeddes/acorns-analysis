...
selections<-selections.raw[!(is.na(selections.raw$Method)), -c(1:4)]
dim(selections)
head(selections)
colSums(selections, na.rm=T)
...
# First column
var.mat[2:8,1] <- colSums(selections, na.rm=T)[2:8]
...
for(j in jstart:9) {
  cat(jstart, '\n')
  varj <- colnames(selections)[j]
  cat(varj, '\n')
  for (i in (jstart+1):10) {
    vari <- colnames(selections)[i]
    cat(vari, '\n')
    #print(length(which(selections[,vari]==1 & selections[,varj]==1)))
    var.mat[i-2,j-2] <- length(which(selections[,vari]==1 & selections[,varj]==1))
  }
  jstart <- jstart+1
}
...
pairsJSG(statsdata[,4:11], lower.panel=panel.sp, upper.panel=panel.smooth, diag.panel=panel.density, labels=labels, col.smooth = "black")
...
lines(x=1994:2011, y=predict(mod.ols), col=gc[1], lty=2, lwd=2)
lines(x=1994:2011, y=predict(mod.ols4), col=gc[2], lty=3)
lines(x=1994:2011, y=predict(mod.ols.AIC), col=gc[3], lty=4, lwd=2)
lines(x=1994:2011, y=predict(mod.ols.min), col=gc[4], lty=5, lwd=2)
dev.off()
```
...
# Remove models not selectd by AIC or P-value and those that did not retain any terms
