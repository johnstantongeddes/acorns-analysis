...
s_AIC1 <- lmer(acorn_count ~ April_rain_lagged1 + (1|site_name) + (1|year), data=sub_statsdata)
summary(s_AIC1)
...
p1 + geom_pointrange(limits)

p2 <- ggplot(ptest, aes(x=factor(site_name), y=resp, fill=factor(type))) + geom_bar(stat="identity") + facet_grid(. ~ type) 

+ labs(x="Term", y="% of models including term")
...
p1 + geom_bar(limits, position=dodge, width=0.25)
```
